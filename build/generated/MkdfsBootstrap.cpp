...
#define TAG "dk.mikkendigital.mkdfs"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Mkdfs_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Mkdfs.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::MkdfsBindings::lookupGeneratedInit(
...
static void Mkdfs_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Mkdfs_getBinding)->GetFunction());
}

static void Mkdfs_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::MkdfsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MkdfsBinding = {
	"dk.mikkendigital.mkdfs",
	Mkdfs_init,
	Mkdfs_dispose
};
...
Java_dk_mikkendigital_mkdfs_MkdfsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("dk.mikkendigital.mkdfs", &MkdfsBinding);
	titanium::KrollBindings::addExternalLookup(&(::MkdfsBindings::lookupGeneratedInit));
}
